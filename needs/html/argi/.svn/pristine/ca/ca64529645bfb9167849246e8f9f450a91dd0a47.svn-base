...
var STATUS_DESC = (_STATUS_DESC = {}, (0, _defineProperty3.default)(_STATUS_DESC, STATUS_CREATE, '新建'), (0, _defineProperty3.default)(_STATUS_DESC, STATUS_COMMIT, '待审阅'), (0, _defineProperty3.default)(_STATUS_DESC, STATUS_READ, '已阅'), (0, _defineProperty3.default)(_STATUS_DESC, STATUS_DENY, '不通过'), _STATUS_DESC);

exports.default = {
  createStatus: STATUS_CREATE,
  commitStatus: STATUS_COMMIT,
  readStatus: STATUS_READ,
  denyStatus: STATUS_DENY,
  ...
  isReadStatus: function isReadStatus(code) {
    return STATUS_READ === '' + code;
  },
  isDenyStatus: function isDenyStatus(code) {
    return STATUS_DENY === '' + code;
  },
  ...
  //0.临时缓存id
  stageWeeklyId: function stageWeeklyId(idVals) {
    this._setData('idVals', idVals);
  },

  //1.临时缓存本周情况数据
  stageWeeklyList: function stageWeeklyList(weeklyList) {
    this._setData('weeklyList', weeklyList);
  },

  //2.临时缓存下周情况数据
  stageWeekPlanList: function stageWeekPlanList(weekPlanList) {
    this._setData('weekPlanList', weekPlanList);
  },


  //3.临时缓存支援事项数据
  stageSuppertItem: function stageSuppertItem(suppertItem) {
    this._setData('suppertItem', suppertItem);
  },

  //4.临时缓存市场信息数据
  stageMarketLetter: function stageMarketLetter(marketLetter) {
    this._setData('marketLetter', marketLetter);
  },
  ...
  commitData: function commitData(fn) {
    ...
    var weekId = function weekId(_weekId) {
      if (_weekId === 'null') {
        return '';
      }
      return _weekId;
    ...
    for (var i = 0; i < 7; i++) {
      var remark = {};
      remark.nextWeekPlan = data.weekPlanList[i];
      remark.remarks = data.weeklyList[i];
      remark.id = data.idVals[i];
      weeklyRemarks.push(remark);
    }
    var subData = {
      weeklyPublication: { suppertItems: data.suppertItem, marketLetter: data.marketLetter, id: data.weekId, status: '1' },

      weeklyRemarks: weeklyRemarks
    };
    _Ajax2.default.postJson('/workorder/submit_weekly', subData, function (result) {
      self.stageStatus(STATUS_COMMIT);
      fn(subData);
    });
  },
